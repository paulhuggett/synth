...
@interface AppDelegate () {
  ...
  MIDIChangeHandler *midiChangeHandler_;
}
@end
...
- (id)init {
  ...
  if (self) {
    ...
    midiChangeHandler_ = [[MIDIChangeHandler alloc] init];
  }
  return self;
}
...
// read MIDI
// ~~~~~~~~~
static void readMIDI (MIDIPacketList const *const packetList, void *userData,
                      void * /*connRefCon*/) {
...
- (BOOL)application:(UIApplication *)application
  ...
  if (erc == noErr) {
    erc = [self->midiChangeHandler_ startMIDIWithReadProc:readMIDI refCon:(__bridge void *)self];
  }
